...
<div class="dashboard-container">
    <div class="dashboard-card">
        ...
        <div class="dashboard-header">
                ...
                <div class="tabs" style="display: flex; gap: 10px;">
                    <a href="{{ route('co-owner.tenants.index', array_merge(request()->except('status'), ['status' => 'active'])) }}"
                        style="
                            ...
                            background-color: {{ $status === 'active' ? '#70AE48' : '#ECFDF5' }};
                            color: {{ $status === 'active' ? '#ffffff' : '#065F46' }};
                            border: 1px solid #70AE48;
                            box-shadow: {{ $status === 'active' ? '0 4px 10px rgba(112, 174, 72, 0.3)' : 'none' }};
                        "
                        onmouseover="this.style.backgroundColor='{{ $status === 'active' ? '#5d8f3a' : '#70AE48' }}'; this.style.color='#fff'"
                        onmouseout="this.style.backgroundColor='{{ $status === 'active' ? '#70AE48' : '#ECFDF5' }}'; this.style.color='{{ $status === 'active' ? '#fff' : '#065F46' }}'">
                        Actifs
                    </a>
                    ...
                    <a href="{{ route('co-owner.tenants.index', array_merge(request()->except('status'), ['status' => 'archived'])) }}"
                        style="
                            ...
                            text-decoration: none;
                            transition: all 0.2s ease;
                            background-color: {{ $status === 'archived' ? '#70AE48' : '#ECFDF5' }};
                            color: {{ $status === 'archived' ? '#ffffff' : '#065F46' }};
                            border: 1px solid #70AE48;
                            box-shadow: {{ $status === 'archived' ? '0 4px 10px rgba(112, 174, 72, 0.3)' : 'none' }};
                        "
                        onmouseover="this.style.backgroundColor='{{ $status === 'archived' ? '#5d8f3a' : '#70AE48' }}'; this.style.color='#fff'"
                        onmouseout="this.style.backgroundColor='{{ $status === 'archived' ? '#70AE48' : '#ECFDF5' }}'; this.style.color='{{ $status === 'archived' ? '#fff' : '#065F46' }}'">
                        Archives
                    </a>
                </div>
            </div>
            ...
            @if (session('success'))
                <div
                    style="margin-bottom: 1rem; background: rgba(112, 174, 72, 0.1); border: 1px solid rgba(112, 174, 72, 0.3); border-radius: 14px; padding: 12px 14px; color: #2e5e1e; font-weight: 950; display: flex; align-items: center; gap: 10px;">
