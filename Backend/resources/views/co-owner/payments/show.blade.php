...
<div class="payment-detail">
    ...
    <div class="page-header">
        ...
        <div class="header-actions">
            ...
            <button onclick="showSendReceiptModal({{ $payment->id }}, '{{ $payment->lease->tenant->user->full_name ?? $payment->lease->tenant->user->name ?? 'Locataire' }}', '{{ $payment->lease->tenant->user->email ?? '' }}')" class="btn-primary" style="background-color: #0b7dda;">
    ...
    <!-- Statut Banner -->
    <div class="status-banner {{ $payment->status === 'approved' ? 'success' : ($payment->status === 'pending' ? 'warning' : 'danger') }}">
        ...
        <div class="status-content">
            <h3>Paiement {{ $payment->status === 'approved' ? 'confirmé' : ($payment->status === 'pending' ? 'en attente' : 'annulé') }}</h3>
        ...
        <div class="detail-card">
            ...
            <div class="tenant-info">
                <div class="tenant-avatar">
                    {{ substr($payment->lease->tenant->user->first_name ?? 'N', 0, 1) }}{{ substr($payment->lease->tenant->user->last_name ?? 'A', 0, 1) }}
                </div>
                <div class="tenant-details">
                    <h3>{{ $payment->lease->tenant->user->full_name ?? 'Non renseigné' }}</h3>
            ...
            <div class="lease-info">
                ...
                <p><strong>Début:</strong> {{ $payment->lease->start_date ? $payment->lease->start_date->format('d/m/Y') : '-' }}</p>
                <p><strong>Fin:</strong> {{ $payment->lease->end_date ? $payment->lease->end_date->format('d/m/Y') : '-' }}</p>
                <p><strong>Loyer mensuel:</strong> {{ number_format($payment->lease->rent_amount, 0, ',', ' ') }} FCFA</p>
            </div>
        </div>
    </div>
    ...
    <div class="notes-card">
        ...
        <div class="notes-meta">
            Enregistré par {{ $payment->landlord->name ?? 'Utilisateur' }} le {{ $payment->created_at->format('d/m/Y à H:i') }}
        </div>
    </div>
    @endif
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="modal-info">
                ...
                <div class="info-row">
                    <span class="info-label">Locataire :</span>
                    <span class="info-value">{{ $payment->lease->tenant->user->full_name ?? $payment->lease->tenant->user->name ?? 'N/A' }}</span>
                </div>
            </div>
            <div class="modal-message warning-message">
                <span class="warning-icon">⚠️</span>
                <p>Cette action déplacera ce paiement dans les archives. Vous pourrez toujours le consulter ultérieurement.</p>
            </div>
        </div>
        <div class="modal-footer">
            <button class="btn-cancel" onclick="closeModal('archiveModal')">Annuler</button>
            <form id="archiveForm" action="{{ route('co-owner.payments.archive', $payment->id) }}" method="POST" style="display: inline;">
                @csrf
                @method('PUT')
