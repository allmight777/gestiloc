        ...
        <form method="GET" action="{{ route('co-owner.quittances.index') }}" id="filter-form">
            ...
            <div style="display: flex; gap: 0.75rem; margin-bottom: 2rem;">
                ...
                <button type="submit" name="status" value="sent"
                        ...
                        style="padding: 10px 24px; border: none; border-radius: 20px; font-weight: 600; font-size: 0.9rem; cursor: pointer; transition: all 0.2s; background: {{ $statusFilter === 'sent' ? '#3B82F6' : '#E5E7EB' }}; color: {{ $statusFilter === 'sent' ? 'white' : '#6B7280' }};">
                    À envoyer
                </button>
                ...
                <button type="submit" name="status" value="year"
                        ...
                        style="padding: 10px 24px; border: none; border-radius: 20px; font-weight: 600; font-size: 0.9rem; cursor: pointer; transition: all 0.2s; background: {{ $statusFilter === 'year' ? '#3B82F6' : '#E5E7EB' }}; color: {{ $statusFilter === 'year' ? 'white' : '#6B7280' }};">
                    Par an
                </button>
            </div>
                ...
                @foreach($receipts as $receipt)
                    <div style="background: white; border-radius: 12px; padding: 1.5rem; border: 1px solid #E5E7EB; transition: all 0.2s; cursor: pointer;"
                        ...
                        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 1rem;">
                            @if($receipt->status == 'issued')
                                <div style="display: inline-flex; align-items: center; gap: 6px; padding: 4px 12px; background: #DBEAFE; border-radius: 20px;">
                                    ...
                                    <span style="color: #1D4ED8; font-size: 0.75rem; font-weight: 700; text-transform: uppercase;">Envoyée le {{ \Carbon\Carbon::parse($receipt->issued_date)->format('d M Y') }}</span>
                                </div>
                            @elseif($receipt->status == 'pending')
                                <div style="display: inline-flex; align-items: center; gap: 6px; padding: 4px 12px; background: #FEF3C7; border-radius: 20px;">
                                    ...
                                    <span style="color: #D97706; font-size: 0.75rem; font-weight: 700; text-transform: uppercase;">En attente d'envoi</span>
                                </div>
                            @else
                                <div style="display: inline-flex; align-items: center; gap: 6px; padding: 4px 12px; background: #FEE2E2; border-radius: 20px;">
                                    ...
                                    <span style="color: #DC2626; font-size: 0.75rem; font-weight: 700; text-transform: uppercase;">En attente d'envoi</span>
                                </div>
                            @endif
                        </div>
                        ...
                        <div style="display: flex; align-items: center; gap: 6px; color: #6B7280; font-size: 0.85rem; margin-bottom: 1.5rem;">
                            ...
                            <span>{{ $receipt->tenant->first_name ?? '' }} {{ $receipt->tenant->last_name ?? '' }} - {{ $receipt->property->city ?? '' }}</span>
                        </div>
                        ...
                        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem; margin-bottom: 1.5rem; padding-bottom: 1.5rem; border-bottom: 1px solid #F3F4F6;">
                            <div>
                                ...
                                <div style="font-weight: 700; color: #1F2937; margin-bottom: 0.25rem;">
                                    {{ \Carbon\Carbon::parse($receipt->paid_month . '-01')->locale('fr')->translatedFormat('F Y') }}
                                </div>
                                <div style="color: #9CA3AF; font-size: 0.8rem;">
                                    {{ \Carbon\Carbon::parse($receipt->issued_date)->format('d M Y') }}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div style="font-weight: 700; color: #1F2937; margin-bottom: 0.25rem;">
                                    {{ number_format(($receipt->amount_paid - ($receipt->lease->charges_amount ?? 0)) * 655, 0, ',', ' ') }} FCFA
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div style="font-weight: 700; color: #1F2937;">
                                    {{ number_format(($receipt->lease->charges_amount ?? 0) * 655, 0, ',', ' ') }} FCFA
                                </div>
                            </div>
                        </div>
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
                            <div>
                                ...
                                <div style="font-size: 1.5rem; font-weight: 700; color: #3B82F6;">
                                    {{ number_format($receipt->amount_paid * 655, 0, ',', ' ') }} FCFA
                                </div>
                                <div style="color: #3B82F6; font-size: 0.8rem; font-weight: 600;">
                                    Crédit le {{ \Carbon\Carbon::parse($receipt->issued_date)->format('d M Y') }}
                                </div>
                            </div>
                        </div>
                        ...
                        <div style="display: flex; gap: 0.5rem;">
                            ...
                            <form action="{{ route('co-owner.quittances.send-email', $receipt->id) }}" method="POST" style="flex: 1;">
                                @csrf
                                <button type="submit"
                                        style="width: 100%; display: inline-flex; align-items: center; justify-content: center; gap: 6px; padding: 10px; background: white; color: #6B7280; border: 1px solid #E5E7EB; border-radius: 8px; font-weight: 600; font-size: 0.85rem; cursor: pointer; transition: all 0.2s;"
                                        onmouseover="this.style.background='#F9FAFB'"
                                        onmouseout="this.style.background='white'">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid #F3F4F6; color: #9CA3AF; font-size: 0.8rem; text-align: center;">
                            Créé le {{ \Carbon\Carbon::parse($receipt->created_at)->format('d M Y') }}
                        </div>
                    </div>
                @endforeach
            </div>
