            ...
            return DB::transaction(function () use ($data, $invitation, $ref) {
                ...
                $tenant = Tenant::firstOrCreate(
                    ...
                    [
                        'first_name'     => $firstName,
                        'last_name'      => $lastName,
                        'status'         => 'active', // ✅ Changé de 'candidate' à 'active' lors de l'acceptation
                        'solvency_score' => 0,
                        'meta'           => [
                ...
                // ✅ Si le Tenant existait déjà avec status 'candidate', le passer à 'active'
                if ($tenant->status === 'candidate') {
                    $tenant->status = 'active';
                    $tenant->save();
                }

                // 4) Marquer l'invitation comme acceptée ET mettre à jour le meta du Tenant
                $invitation->accepted_at    = now();
                $invitation->tenant_user_id = $user->id;
                $invitation->save();

                // ✅ Mettre à jour le meta du Tenant avec la date d'acceptation
                $meta                = $tenant->meta ?? [];
                $meta['accepted_at'] = now()->toISOString();
                $tenant->meta        = $meta;
                $tenant->save();
