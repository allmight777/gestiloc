...
use App\Notifications\LeaseCreatedLandlord;
use App\Notifications\LeaseCreatedTenant;
...
class LeaseController extends Controller
{
    ...
    public function store(StoreLeaseRequest $request)
    {
        ...
        return DB::transaction(function () use ($data, $property) {
            ...
            if ($totalFirstInvoice > 0) {
                ...
                // Notifier la création de la facture au locataire et au propriétaire
                $lease->tenant->user->notify(new InvoiceCreated($invoice));
                $this->getLandlord()->user->notify(new InvoiceCreated($invoice));
            }

            // TODO: Déclencher ici un Job pour générer le PDF du contrat (Queue)
            // GenerateLeasePdfJob::dispatch($lease);

            // Notifier le propriétaire et le locataire
            $this->getLandlord()->user->notify(new LeaseCreatedLandlord($lease));
            $lease->tenant->user->notify(new LeaseCreatedTenant($lease));
    ...
    public function terminate(TerminateLeaseRequest $request, $uuid)
    {
        ...
        // Notifier bailleur et locataire
        $this->getLandlord()->user->notify(new LeaseTerminated($lease));
        $lease->tenant->user->notify(new LeaseTerminated($lease));
